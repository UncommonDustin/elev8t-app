...
When suggesting tasks, return them as a JSON array of objects with keys: text, category (one of: Fitness, Mindset, Business, Family, Discipline).`;

export default function ELEV8TApp() {
  const [tab, setTab] = useState("power");
  const [tasks, setTasks] = useState(() => load("elev8t_tasks_" + todayKey(), defaultTasks()));
  const [points, setPoints] = useState(() => load("elev8t_points", 0));
  const [streak, setStreak] = useState(() => load("elev8t_streak", 0));
  const [totalWins, setTotalWins] = useState(() => load("elev8t_wins", 0));
  const [todayWon, setTodayWon] = useState(() => load("elev8t_won_" + todayKey(), false));
  const [showWin, setShowWin] = useState(false);
  const [journalText, setJournalText] = useState(() => load("elev8t_journal_" + todayKey(), ""));
  const [journalPrompt] = useState(() => JOURNAL_PROMPTS[new Date().getDay() % JOURNAL_PROMPTS.length]);
  const [photos, setPhotos] = useState(() => load("elev8t_photos", []));
  const [challenges, setChallenges] = useState(() => load("elev8t_challenges", []));
  const [weekHistory, setWeekHistory] = useState(() => load("elev8t_week_" + weekKey(), {}));
  ...
  const [coachMessages, setCoachMessages] = useState(() => load("elev8t_coach_msgs", []));
  ...
  useEffect(() => { save("elev8t_tasks_" + todayKey(), tasks); }, [tasks]);
  useEffect(() => { save("elev8t_journal_" + todayKey(), journalText); }, [journalText]);
  useEffect(() => { save("elev8t_photos", photos); }, [photos]);
  useEffect(() => { save("elev8t_challenges", challenges); }, [challenges]);
  useEffect(() => { save("elev8t_coach_msgs", coachMessages.slice(-40)); }, [coachMessages]);

  useEffect(() => {
    const today = todayKey();
    const updated = { ...weekHistory, [today]: { completed: completedCount, total: tasks.length, xp: todayXP, won: allDone } };
    setWeekHistory(updated);
    save("elev8t_week_" + weekKey(), updated);
  ...
  useEffect(() => {
    if (allDone && !todayWon) {
      ...
      save("elev8t_points", np); save("elev8t_streak", ns); save("elev8t_wins", nw);
      save("elev8t_won_" + todayKey(), true);
      setShowWin(true);
  ...
  function saveEdit(id) {
    setTasks(p => p.map(t => t.id === id ? { ...t, text: editVal } : t));
    setEditingId(null);
  }
  ...
  async function getSuggestions() {
    setAiLoading(true);
    try {
      const prompt = `Generate exactly 5 daily Power List tasks for Dustin for today. 
      Goals: ${USER_GOALS.join("; ")}. 
      Make them specific, actionable, and achievable in one day.
      Return ONLY a JSON array, no markdown, no explanation. Example format:
      [{"text":"Task description","category":"Fitness"}]
      Categories must be one of: Fitness, Mindset, Business, Family, Discipline.`;
      const raw = await callClaude([{ role: "user", content: prompt }], COACH_SYSTEM, 600);
      const clean = raw.replace(/```json|```/g, "").trim();
      const parsed = JSON.parse(clean);
      if (Array.isArray(parsed)) {
        setTasks(parsed.slice(0, 5).map((t, i) => ({ id: Date.now() + i, text: t.text, category: t.category || "Discipline", completed: false })));
      }
  ...
  async function sendCoachMessage() {
    ...
    setCoachTyping(true);
    try {
      const context = `Today's tasks: ${tasks.map(t => `${t.text} (${t.completed ? "✓" : "✗"})`).join(", ")}. 
      Streak: ${streak} days. Total XP: ${points}. Active challenges: ${challenges.filter(c => c.active).map(c => c.name).join(", ") || "none"}.`;
      const systemWithContext = COACH_SYSTEM + "\n\nCurrent context: " + context;
      const history = newMsgs.slice(-10).map(m => ({ role: m.role, content: m.content }));
      const reply = await callClaude(history, systemWithContext, 800);
      setCoachMessages(p => [...p, { role: "assistant", content: reply }]);
    } catch {
      setCoachMessages(p => [...p, { role: "assistant", content: "Something went wrong. Try again." }]);
    }
  ...
  async function initCoach() {
    ...
    setCoachTyping(true);
    try {
      const intro = `Greet Dustin briefly (1-2 sentences max). Reference his goals and today's date. Be direct. Then ask what he needs help with today.`;
      const reply = await callClaude([{ role: "user", content: intro }], COACH_SYSTEM, 200);
      setCoachMessages([{ role: "assistant", content: reply }]);
    } catch {
      setCoachMessages([{ role: "assistant", content: `What's up Dustin. What do you need to work through today?` }]);
    }
    setCoachTyping(false);
  }
  ...
  function updatePhotoNote(id, note) {
    setPhotos(p => p.map(ph => ph.id === id ? { ...ph, note } : ph));
  }
  ...
  function createChallenge() {
    ...
    const c = {
      id: Date.now(), name: newChallengeName.trim(), desc: newChallengeDesc.trim(),
      duration: parseInt(newChallengeDays) || 30, currentDay: 1, active: true,
      startDate: todayKey(), lastChecked: null,
    };
    setChallenges(p => [...p, c]);
    setNewChallengeName(""); setNewChallengeDesc(""); setNewChallengeDays(30);
  ...
  function markChallengeDay(id) {
    setChallenges(p => p.map(c => {
      if (c.id !== id) return c;
      if (c.lastChecked === todayKey()) return c;
  ...
  function getWeekDays() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({
        key, label: d.toLocaleDateString("en-US", { weekday: "short" }),
        date: d.toLocaleDateString("en-US", { month: "numeric", day: "numeric" }),
        data: weekHistory[key] || null,
        isToday: key === todayKey(),
        isFuture: d > now && key !== todayKey(),
      });
    }
    return days;
  }
            ...
            {addingTask && (
              <div style={s.addBox}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <select value={newTaskCat} onChange={e => setNewTaskCat(e.target.value)} style={s.select}>
        ...
        {tab === "coach" && (
          <div style={s.panel}>
            <div style={s.panelHeader}>
              ...
              <button onClick={() => { setCoachMessages([]); save("elev8t_coach_msgs", []); }} style={s.btnGhost}>CLEAR</button>
            </div>
            ...
            <div style={s.chatInputWrap}>
              <input ref={coachInputRef} value={coachInput} onChange={e => setCoachInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendCoachMessage()} placeholder="Ask your coach anything..." style={{ ...s.input, flex: 1 }} disabled={coachTyping} />
        ...
        {tab === "journal" && (
          <div style={s.panel}>
            <div style={s.panelHeader}>
              ...
              <div style={s.autoSaveTag}>AUTO-SAVED</div>
            </div>
            <div style={s.promptCard}>
              <div style={s.promptLabel}>TODAY'S PROMPT</div>
              <div style={s.promptText}>"{journalPrompt}"</div>
            </div>
            <textarea value={journalText} onChange={e => setJournalText(e.target.value)} placeholder="Write honestly. No one's watching. Own it..." style={s.textarea} />
            ...
            {addingChallenge && (
              <div style={s.addBox}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8, alignItems: "center" }}>
                  ...
                  <button onClick={createChallenge} style={s.btnAccent}>CREATE CHALLENGE</button>
                </div>
              </div>
            )}
              ...
              {weekDays.map(d => (
                <div key={d.key} style={{ ...s.weekDay, ...(d.isToday ? s.weekDayToday : {}), ...(d.isFuture ? s.weekDayFuture : {}) }}>
                  ...
                  {d.data && !d.isFuture ? (
                    ...
                    <div style={{ fontSize: 20, margin: "6px 0", opacity: 0.3 }}>○</div>
                  )}
                </div>
              ))}
            </div>
...
const styles = {
  ...
  winOverlay: { position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.92)", animation: "fadeIn .3s ease" },
  ...
  headerEyebrow: { fontSize: 10, letterSpacing: 5, color: "#f97316", marginBottom: 4, textTransform: "uppercase" },
  ...
  autoSaveTag: { fontSize: 9, letterSpacing: 2, color: "#333" },
